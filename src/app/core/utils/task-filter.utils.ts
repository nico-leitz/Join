...
export function filterTasks(tasks: Task[], options: TaskFilterOptions): Task[] {
  const searchedTasks = filterTasksBySearchTerm(tasks, options.searchTerm ?? '');

  return filterTasksByStatus(searchedTasks, options.status);
}
...
export function filterTasksBySearchTerm(tasks: Task[], searchTerm: string): Task[] {
  ...
  return tasks.filter((task) => matchesTaskSearch(task, normalizedSearchTerm));
}
 ...
 */
function matchesTaskSearch(task: Task, searchTerm: string): boolean {
  return (

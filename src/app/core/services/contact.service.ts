...
import { Contact, ContactRow, CreateContact, UpdateContact } from '../models/contact.model';
...
export class ContactService {
  ...
  async updateContact(id: string, contact: UpdateContact): Promise<Contact> {
    ...
    const updatedRow = this.requireMutationRow(data as ContactRow | null, error, 'updated');
    const updatedContact = this.mapContactRow(updatedRow);
  ...
  async deleteContact(id: string): Promise<void> {
    ...
    this.requireMutationRow(data, error, 'deleted');
   ...
   */
  private createInsertPayload(contact: CreateContact, badgeColor: string): Partial<ContactRow> {
    return {
  ...
  private createUpdatePayload(contact: UpdateContact): Partial<ContactRow> {
    const payload: Partial<ContactRow> = { updated_at: new Date().toISOString() };
    if (contact.firstName !== undefined) payload.first_name = contact.firstName.trim();
    if (contact.lastName !== undefined) payload.last_name = contact.lastName.trim();
    if (contact.email !== undefined) payload.email = contact.email.trim();
    if (contact.phone !== undefined) payload.phone = contact.phone?.trim() || null;
    return payload;
  }
   ...
   */
  private requireMutationRow<T>(data: T | null, error: unknown, action: 'updated' | 'deleted'): T {
    if (error) {
      throw error;
    }

    if (!data) {
      throw new Error(`Contact could not be ${action}.`);
    }
  ...
  private async getUsedBadgeColors(): Promise<Set<string>> {
    const { data, error } = await this.supabase.from(this.tableName).select('badge_color');

...
export class AddTaskContent implements OnInit, OnDestroy {
  ...
  toggleContactsMenu(): void {
    ...
    this.contactsMenuOpen.update((isOpen) => {
      return !isOpen;
    });
  }
  ...
  toggleCategoryMenu(): void {
    ...
    this.categoryMenuOpen.update((isOpen) => {
      return !isOpen;
    });
  ...
  private async loadTasks(): Promise<void> {
    if (this.taskService.allTasks().length > 0) {
      return;
    }
  ...
  private closeUnrelatedMenus(target: Element): void {
    if (!target.closest('.add-task__contact-select')) {
      this.contactsMenuOpen.set(false);
    }

    if (!target.closest('.add-task__category-select')) {
      this.categoryMenuOpen.set(false);
    }
  }
  ...
  private replaceSubtaskTitle(index: number, title: string): void {
    this.draftSubtasks.update((subtasks) => {
      return subtasks.map((subtask, currentIndex) => {
        return currentIndex === index ? { title } : subtask;
      });
    });
  }
  ...
  private getNextSortOrder(): number {
    const statusTasks = this.taskService.allTasks().filter((task) => {

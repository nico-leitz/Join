...
export class BoardDataStateService {
  ...
  readonly filteredTasks = computed(() => {
    return filterTasksBySearchTerm(this.taskService.allTasks(), this.searchTerm());
  });

  /** Filtered tasks belonging to the to-do column. */
  readonly todo = computed(() => {
    return filterTasksByStatus(this.filteredTasks(), 'todo');
  });

  /** Filtered tasks belonging to the in-progress column. */
  readonly inProgress = computed(() => {
    return filterTasksByStatus(this.filteredTasks(), 'in_progress');
  });

  /** Filtered tasks belonging to the awaiting-feedback column. */
  readonly awaitFeedback = computed(() => {
    return filterTasksByStatus(this.filteredTasks(), 'awaiting_feedback');
  });

  /** Filtered tasks belonging to the done column. */
  readonly done = computed(() => {
    return filterTasksByStatus(this.filteredTasks(), 'done');
  });
  ...
  async load(): Promise<void> {
    const [, boardData, contacts] = await Promise.all([
      this.taskService.getTasks(),
      this.taskService.loadAllBoardData(),
      this.contactService.getContacts(),
    ]);

    this.setRelations(boardData.subtasks, boardData.assignments);
  ...
  async refreshRelations(): Promise<void> {
    const boardData = await this.taskService.loadAllBoardData();
  ...
  getContacts(taskId: string): Contact[] {
    const contactIds = this.contactIdsByTaskId().get(taskId) ?? [];
  ...
  updateSubtask(updatedSubtask: Subtask): void {
    this.allSubtasks.update((subtasks) => {
      return replaceBoardSubtask(subtasks, updatedSubtask);
    });
  }
   ...
   */
  private setRelations(subtasks: Subtask[], assignments: TaskAssignmentRow[]): void {

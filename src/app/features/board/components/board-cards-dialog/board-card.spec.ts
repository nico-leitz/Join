...
describe("BoardCardsDialog Component", () => {
  ...
  it("should compute the correct contactSelectionLabel based on selected contacts count", () => {
    ...
    expect(component.contactSelectionLabel()).toBe(
  ...
  it("should allow adding, updating, and removing editable subtasks", () => {
    ...
    expect(component.editableSubtasks()[1].title).toBe(
      "New Draft Subtask",
    );
    expect(component.newSubtaskTitle()).toBe("");
    ...
    component.updateEditingSubtaskTitle(mockInputEvent);
  ...
  it("should start editing when the subtask title is clicked", () => {
    ...
    expect(
      fixture.nativeElement.querySelector(
        ".board_dialog__subtask_list_input",
      ),
    ).toBeTruthy();
  });
  ...
  it("should toggle subtask completion via service and emit updated subtask", async () => {
    ...
    mockTaskService.toggleSubtaskCompletion.mockResolvedValue(
      updatedMock,
    );
    ...
    expect(
      mockTaskService.toggleSubtaskCompletion,
    ).toHaveBeenCalledWith("sub-1", true);
    expect(component.subtaskUpdated.emit).toHaveBeenCalledWith(
      updatedMock,
    );
    expect(component.errorMessage()).toBe("");
  });
  ...
  it("should handle subtask toggle failure by reverting checkbox and showing error", async () => {
    mockTaskService.toggleSubtaskCompletion.mockRejectedValue(
    ...
    expect(
      (mockEvent.target as HTMLInputElement).checked,
    ).toBe(false);
    expect(component.errorMessage()).toBe(
  ...
  it("should reject saving if edit form is invalid", async () => {
    ...
    expect(
      mockTaskService.updateTaskWithRelations,
  ...
  it("should format payload, save changes via service, and emit taskUpdated event", async () => {
    ...
    mockTaskService.updateTaskWithRelations.mockResolvedValue(
      updatedTaskResponse,
    );
  ...
  it("should gracefully handle API errors during saveTask", async () => {
    mockTaskService.updateTaskWithRelations.mockRejectedValue(
